...
if(int_config_input == 1):
	print("\nConfiguration 1, Volt divider Multiple Resistors in Series, Same R\n");
	...
	for n in range(len(output_volt_array)):
		if(n == 0):
			...
			output_volt_array[n] = (( n*R)/ ( ( n*R) + R_track))*Vsource;
	...
	plt.title('Configuration 1(Series R to gnd, Same, volt divider): Output voltage vs number of bends');
...
if(int_config_input == 2):
	print("\nConfiguration 2, Volt divider Multiple Resistors in Series, Same R\n");
	...
	for n in range(len(output_volt_array)):
		if(n == 0):
			...
			output_volt_array[n] = ( R_track / ( (n*R) + R_track))*Vsource;
	...
	#Make plot from array of output voltage and array of bends
	plt.plot(bends_array,output_volt_array);
	plt.title('Configuration 2(Series R to Vsource, Same, volt divider): Output voltage vs number of bends');
	plt.xlabel("Bends, # of Resistors connected, decreasing resistance with increasing # of bends");
	plt.ylabel("Output Voltage (V)");
	plt.xlim(0,num_resistor_combo)  # decreasing resistance, increasing # of bends
	plt.xticks(bends_array);
